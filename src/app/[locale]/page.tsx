...
export default async function Home() {
  ...
  const linkStyle = "text-xl flex gap-2 items-center justify-center px-5 py-3 rounded-full cursor-pointer"
    ...
    <main className = "pb-20 flex min-h-screen flex-col items-center justify-start pt-25 cursor-default select-none bg-[#e3e3e1] text-[#121212] dark:bg-[#121212] dark:text-[#e3e3e1]">
      ...
      <div className = "grid grid-flow-col auto-cols-[minmax(max-content,1fr)] gap-10">
        <Link
          ...
          target = "_blank"
          className = {`${linkStyle} font-googlesanscode-light border-2 border-[#121212] dark:border-[#e3e3e1] relative overflow-hidden group transition-all duration-350`}
        >
          ...
          <IconCode className = "w-6 h-6 relative z-10 group-hover:text-[#e3e3e1]" />
          <span className = "relative z-10 group-hover:text-[#e3e3e1]">GitHub</span>
        </Link>
